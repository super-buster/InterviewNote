...
短期计划：1.  计划做端到端的RL训练，尝试类似kimi-researcher那种方式对整个轨迹都做强化学习。2. 转为multi-agent方案。难点是agent之间的通信，可以类似qwen-agent分配任务，或者openhands基于事件驱动，消息订阅。multi-agent的方案好处是可以加强信息收集的能力。例如构建一个搜索agent，参考jina方式，构建一个FIFO队列，每次动态生成query。 3. 工具建设。搜索数据太长，使用embedding取片段。think拆分工具。4. 尝试GRPO应用到答案生成，难点是评价答案的好坏，可以参考BRPO。（SPCT评估模板，对答案进行pair-wise打分，reference随机选择一个group的几个数据）

长期计划：1. 和ifind终端的数据紧密融合在一起，最好完善量化策略那一块。这是我们后面可以产生巨大价值的点 2. 把我们的流程做成一个系统的框架，记忆，规划，代码沙盒，流程都独立出来组内统一使用。3. 操作终端。
