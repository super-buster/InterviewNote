...
1. 用户query合成
   ...
   为了模型sft的效果，query要多样，需要选择不同类型的query合成数据。冷启动使用产品提供的query，然后进行意图分解，把原始query+意图（动态生成150+）作为seed，生成多样性更大的query。之后让大模型判断难度，分为简单（查指标，新闻），中等（ 财报解读），困难（荐股，预测），大概按照60%，30%，10%的比例筛选3000条，用于后续workflow合成数据。多轮query尝试合成，但是和用户问的差距太大，因此直接用线上的多轮。
