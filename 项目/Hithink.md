...
3. 正文生成
   ...
   我们会用多个模型，包括qwen72b（后面换成qwen-2.5 72b），R1生成多份答案。之后根据模型的pair-wise评分选择分数最好的，分成几个维度：准确性，逻辑性，专业性，引用正确性。然后过滤掉一些答案比较短的数据，控制答案在1000-1500字符左右（太长容易幻觉）。
...
1. 模型幻觉问题
   ...
   广义上幻觉就是模型回答错误，胡说八道，狭义的幻觉是指模型本身有这方面的只是，但是经过alignment处理之后就回答不对了。目前来说幻觉属于无法根除的问题，通常都是从数据层面缓解幻觉现象。
