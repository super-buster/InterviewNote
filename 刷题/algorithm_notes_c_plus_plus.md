...
\|A\|+\|B\|+\|C\|-\|A^B^\|-\|A^C^\|-\|B^C^\|+\|A^BC^\|。在计算欧拉phi函数时会用到这个理论。假设\$n=p^e1^\$，那么n和p，2p，3p，...都不互素，于是\$φ
(n)=p~1~^e1^-p~1~^e1^/p~1~ =
p~1~^e1^(1-`\frac{1}{p_1}`{=latex})\$。假设\$n=p~1~^e1^p~2~^e2^\$，那么根据容斥原理，$\phi (n)=n- \frac{n}{p_1^{e1}}- \frac{n}{p_2^{e2}}+\frac{n}{p_1p_2}= n(1-\frac{1}{p_1}})(1-\frac{1}{p_2}})$
